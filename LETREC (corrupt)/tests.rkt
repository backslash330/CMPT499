...
(check-equal? (run "letrec f(x) = if zero?(x)  then 0 else -((f -(x,1)), -2) in (f 4)") (num-val 8))
(check-equal? (run "let m = -5 in letrec f(x) = if zero?(x) then 0 else -((f -(x,1)), m) in (f 4)") (num-val 20))
